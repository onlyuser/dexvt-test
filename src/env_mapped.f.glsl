...
{
    vec3 reflected_camera_dir = reflect(camera_direction, surface_normal);
        ...
        inout vec3        refracted_camera_dir)
{
    ...
    vec3 refracted_camera_dirB = refract(camera_direction, surface_normal, eta+eta_rgb_offset);
...
void main(void) {
    ...
    float eta_red = AIR_REFRACTIVE_INDEX/(GLASS_REFRACTIVE_INDEX + GLASS_REFRACTIVE_INDEX_RGB_OFFSET);
    float eta_rgb_offset = eta - eta_red;
    ...
    //vec3 ray_plane_isect = lerp_vertex_position_world + refracted_camera_dir*???;
