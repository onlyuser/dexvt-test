                             ...
                             glm::vec3* reflected_ray)
{
    ...
    if(reflected_ray) {
        *reflected_ray = glm::reflect(ray_dir, plane_normal);
    }
    return result;
}
                              ...
                              glm::vec3* intersection_normal)
{
    ...
    glm::vec3 to_ray_from_sphere = ray_nearest_point_to_sphere - sphere_origin;
    float distance_to_ray_from_sphere = glm::length(to_ray_from_sphere);
    if(distance_to_ray_from_sphere > sphere_radius ||                    // ray entirely misses sphere
       glm::dot(sphere_origin, ray_dir) < glm::dot(ray_origin, ray_dir)) // ray points away from sphere
    {
        return BIG_NUMBER;
    }
    float half_length_inside_sphere = pow(pow(sphere_radius, 2) - pow(distance_to_ray_from_sphere, 2), 0.5);
    glm::vec3 intersection = ray_nearest_point_to_sphere + safe_normalize(ray_origin - ray_nearest_point_to_sphere) * half_length_inside_sphere;
    glm::vec3 _intersection_normal = safe_normalize(intersection - sphere_origin);
    if(reflected_ray) {
        *reflected_ray = glm::reflect(ray_dir, _intersection_normal);
    }
    if(intersection_normal) {
        *intersection_normal = _intersection_normal;
    }
    return glm::distance(ray_origin, intersection);
}
