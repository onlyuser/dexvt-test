...
void map_ndc_depth_to_world_depth(in    float z_near, // camera near-clipping plane distance from camera
...
void gen_basis_transform_from_random_vec(inout mat3 tbn_transform,
                                         in    vec3 normal,
                                         in    vec3 random_vec)
{
...
void main()
{
    ...
    mat3 tbn_transform;
    vec3 random_unit_normal = normalize(texture2D(random_texture, flipped_texcoord).rgb) * 2 - vec3(1); // map from [0, 1] to [-1, 1]
    ...
    for(int i = 0; i < NUM_SSAO_SAMPLE_KERNELS; i++) {
        ...
        float sample_world_linear_depth = abs(dot(sample_world - camera_pos, camera_dir));
