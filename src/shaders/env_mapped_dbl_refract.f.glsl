...
void map_depth_to_actual_depth(in    float z_near, // camera near-clipping plane distance from camera
...
void intersect_ray_plane(in    vec3  orig,                       // point on ray   (I0)
...
void refract_into_env_map(in    vec3        ray_direction,
                          in    vec3        surface_normal,
                          in    float       eta,
                          in    samplerCube env_map_texture,
...
void refract_into_env_map_ex(in    vec3        ray_direction,
                             in    vec3        surface_normal,
                             in    float       eta,
...
void unproject_fragment(in    vec3  frag_pos,
...
void newtons_method_update(in    sampler2D _backface_depth_overlay_texture,
                           ...
                           inout bool      abort)
{
    ...
    intersect_ray_plane(orig,                        // point on ray
                        dir,                         // ray direction
...
void calculate_light_contrib(in    vec3 camera_direction,
                             in    vec3 plane_orig,
...
void main()
{
    ...
    vec3 normal_surface = mix(vec3(0, 0, 1), normalize(vec3(texture2D(bump_texture, flipped_texcoord))), BUMP_FACTOR);
    vec3 normal = normalize(lerp_tbn_transform * normal_surface);

    vec4 frontface_light_contrib;
    calculate_light_contrib(camera_direction,
    ...
    refract_into_env_map_ex(-camera_direction,
                            normal,
                            frontface_eta,
    ...
    //    float frontface_fresnel_reflectance = pow(1 - clamp(dot(camera_direction, normal), 0, 1), FRESNEL_REFLECTANCE_SHARPNESS);
    //    gl_FragColor = mix(frontface_refracted_color, reflected_color, max(reflect_to_refract_ratio, frontface_fresnel_reflectance));
    //    return;
    //}
    ...
    for(int i = 0; i < NUM_NEWTONS_METHOD_ITERS; i++) {
        newtons_method_update(backface_depth_overlay_texture,
    ...
    float backface_fresnel_reflectance = pow(1 - clamp(dot(camera_direction, plane_normal), 0, 1), FRESNEL_REFLECTANCE_SHARPNESS);

    vec4 backface_light_contrib;
    calculate_light_contrib(-normalize(frontface_refracted_camera_dir),
                                       plane_orig,
                                       plane_normal,
                                       backface_light_contrib);

    vec4 inside_color_prelit = mix(MATERIAL_AMBIENT_COLOR, backface_refracted_color, max(beers_law_transmittance, backface_fresnel_reflectance));
    ...
    // frontface fresnel component
    float frontface_fresnel_reflectance = pow(1 - clamp(dot(camera_direction, normal), 0, 1), FRESNEL_REFLECTANCE_SHARPNESS);
    vec4 outside_color_prelit = //mix(vec4(1, 0, 0, 0), vec4(0, 0, 1, 0), frontface_depth) * 0.001 +
                                ...
                                mix(inside_color, reflected_color, max(reflect_to_refract_ratio, frontface_fresnel_reflectance));
