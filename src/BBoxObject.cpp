                                  ...
                                  glm::vec3*       intersection_normal)
{
    ...
    // for each potential separation axis
    glm::mat4 inverse_self_transform = glm::inverse(self_transform_object->get_transform());
    float     nearest_intersection_distance = BIG_NUMBER;
    glm::vec3 _reflected_ray;
    ...
    for(int i = 0; i < 3; i++) {
        ...
        for(int j = 0; j < 2; j++) {
            ...
            glm::vec3 __reflected_ray;
            intersections[k] = ray_plane_intersection(plane_origin,
                                                      plane_normal,
                                                      ray_origin,
                                                      ray_dir,
                                                      reflected_ray ? &__reflected_ray : NULL);
            glm::vec3 point = ray_origin + ray_dir * intersections[k];
            glm::vec3 local_point = glm::vec3(inverse_self_transform * glm::vec4(point, 1));
            if(is_within(local_point)) {
                if(intersections[k] < nearest_intersection_distance) {
                    nearest_intersection_distance = intersections[k];
                    _reflected_ray                = __reflected_ray;
                    _intersection_normal          = plane_normal;
                }
            }
            k++;
        }
    ...
    if(nearest_intersection_distance != BIG_NUMBER) {
        if(intersection_distance) {
            *intersection_distance = nearest_intersection_distance;
        }
        if(reflected_ray) {
            *reflected_ray = _reflected_ray;
        }
        if(intersection_normal) {
            *intersection_normal = _intersection_normal;
        }
        return true;
    }
...
bool BBoxObject::as_sphere_is_ray_intersect(TransformObject* self_transform_object,
                                            ...
                                            glm::vec3*       intersection_normal)
{
    float sphere_radius = fabs(m_max.x - m_min.x) * 0.5f;
    float _intersection_distance = ray_sphere_intersection(self_transform_object->in_abs_system(),
                                                           sphere_radius,
                                                           ray_origin,
                                                           ray_dir,
                                                           reflected_ray,
                                                           intersection_normal);
    if(_intersection_distance == BIG_NUMBER) {
        return false;
    }

    if(intersection_distance) {
        *intersection_distance = _intersection_distance;
    }
